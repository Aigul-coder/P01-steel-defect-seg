#!/usr/bin/env python3
"""Evaluate checkpoint on blind holdout set (excluded from train + val)."""
...
def main() -> None:
    ...
    _, _, holdout_df = prepare_train_val_holdout(
        index,
    ...
    with torch.inference_mode():
        for batch in tqdm(loader, desc="holdout"):
    ...
    out = {
        ...
        "image_ids": holdout_df["ImageId"].astype(str).tolist(),
        **metrics,
    }
    out_path = args.out or (ROOT / cfg["artifacts"]["dir"] / "eval_holdout_metrics.json")
    out_path.parent.mkdir(parents=True, exist_ok=True)
    out_path.write_text(json.dumps(out, indent=2), encoding="utf-8")
    log.info(
        "holdout mean_dice=%.4f mean_iou=%.4f mean_dice_present_only=%.4f n=%s",
        metrics["mean_dice"],
        metrics["mean_iou"],
        metrics.get("mean_dice_present_only", 0.0),
        len(holdout_df),
    )
    log.info(
        "per_class_dice_present_only=%s n_present=%s",
